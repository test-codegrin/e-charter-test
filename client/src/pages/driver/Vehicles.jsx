...
const DriverVehicles = () => {
  ...
  const filterVehicles = () => {
    ...
    // Features filter (AND logic)
    if (selectedFeatures.length > 0) {
      filtered = filtered.filter((vehicle) => {
        if (!vehicle.features) return false;
        return selectedFeatures.every((featureKey) => vehicle.features[featureKey] === 1);
      });
    }
  ...
  // Count vehicles by document status
      ...
      case "rejected":
        return (
          <div className="space-y-1">
            ...
            {statusDescription && (
              <div className="flex items-start space-x-1 mt-1 p-2 bg-red-50 border border-red-200 rounded-md">
                <Info className="w-3 h-3 text-red-600 mt-0.5 flex-shrink-0" />
                <p className="text-xs text-red-700 font-medium">{statusDescription}</p>
              </div>
            )}
          </div>
        );
      ...
      default:
        return (
          <div className="space-y-1">
            ...
            {statusDescription && (
              <div className="flex items-start space-x-1 mt-1 p-2 bg-yellow-50 border border-yellow-200 rounded-md">
                <Info className="w-3 h-3 text-yellow-600 mt-0.5 flex-shrink-0" />
                <p className="text-xs text-yellow-700 font-medium">{statusDescription}</p>
              </div>
            )}
          </div>
        );
  ...
  // Get vehicle features as icons
  const getVehicleFeatureIcons = (features) => {
    if (!features) return null;

    const activeFeatures = availableFeatures.filter(
      (feature) => features[feature.key] === 1
    );
      ...
      <div className="flex items-center flex-wrap gap-1 mt-2">
        {activeFeatures.map((feature) => {
          ...
          return (
            <div
              key={feature.key}
              className="flex items-center space-x-1 px-2 py-1 bg-blue-50 border border-blue-200 rounded-md"
              title={feature.label}
            >
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="fade-in">
        ...
        <p className="text-secondary-600">View and manage your vehicle registrations</p>
      </div>
              ...
              {filteredVehicles.map((vehicle) => {
                ...
                return (
                  ...
                  >
                    <td className="px-6 py-4">
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          ...
                          {/* Feature Icons */}
                          {getVehicleFeatureIcons(vehicle.features)}
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      {getStatusBadge(vehicle.status, vehicle.status_description)}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <Link
                        to={DRIVER_ROUTES.VIEW_VEHICLE + vehicle.vehicle_id}
