...
const registerDriver = asyncHandler(async (req, res) => {
    const { driverName,  email,password, address, cityName, zipCord, phoneNo } = req.body;
    // const file = req.file;

    if (!driverName || !email || !password || !address || !cityName || !zipCord || !phoneNo) {
    ...
    const [existingDriver] = await db.query(driverAuthQueries.driverMailCheck, [email]);
    ...
    const driverValues = [driverName, email, hashedPassword, address, cityName, zipCord, phoneNo];

    const [result] = await db.query(driverAuthQueries.driverInsert, driverValues);

    res.status(201).json({ message: "Driver registered successfully, pending approval", driverId: result.insertId});
});
...
const requestDriverReset = asyncHandler(async (req, res) => {
    ...
    if (!driver || driver.length === 0) return res.status(404).json({ error: "Driver not found" });
...
const verifyDriverResetCode = asyncHandler(async (req, res) => {
    ...
    if (!stored || stored.expires < Date.now()) return res.status(400).json({ error: "Expired or invalid code" });
    if (stored.code !== resetCode) return res.status(400).json({ error: "Incorrect reset code" });
...
const resetDriverPassword = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        const [result] = await db.query(driverAuthQueries.passwordUpdate, [hashed, email]);
...
const updateDriverPassword = asyncHandler(async (req, res) => {
    ...
    const [update] = await db.query(driverAuthQueries.passwordUpdate, [hashedNew, email]);

    if (update.affectedRows === 0) return res.status(500).json({ error: "Password update failed" });
